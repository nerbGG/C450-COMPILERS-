...
class JLogicalAndOp extends JBooleanBinaryExpression {
     ...
     */
    public void codegen(CLEmitter output, String targetLabel, boolean onTrue) {
        if (onTrue) { //both must be true, if lhs is false the whole thing is false
            String falseLabel = output.createLabel();
...
class JNotEqualOp extends JBooleanBinaryExpression {
    ...
    public JExpression analyze(Context context) {
        lhs = (JExpression) lhs.analyze(context);
        rhs = (JExpression) rhs.analyze(context);
        lhs.type().mustMatchOneOf(line(), Type.BOOLEAN, Type.INT);
        rhs.type().mustMatchOneOf(line(),Type.BOOLEAN, Type.INT);
        type = Type.BOOLEAN;
        return this;
    }
