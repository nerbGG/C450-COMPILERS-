...
public class Parser {
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList <String> mods){
        ...
        if(have(EXTENDS)){
            ...
            return new JInterfaceDeclaration(line, mods, name, null, interfaceBody());
        }
    }
    /**
     ...
     */
    ArrayList<JMember> interfaceBody(){//mostly the same as class Body
     ...
     *
     * <pre>
     *   memberDecl ::= IDENTIFIER formalParameters [THROWS qualifiedIndentifier {COMMA qualifiedIndentifier} ] block
     *                | ( VOID | type ) IDENTIFIER formalParameters
     *                              [THROWS qualifiedIndentifier {COMMA qualifiedIndentifier} ] ( block | SEMI )
     ...
     *   interfaceMemberDecl ::=
     *                | ( VOID | type ) IDENTIFIER formalParameters
     *                  [THROWS qualified qualifiedIdentifier {COMMA qualifiedIdentifier}] SEMI
     *                | type variableDeclarators SEMI
     * </pre>
     *
     ...
     */
    private JMember interfaceMemberDecl(ArrayList<String> mods){ //similar to memberDecl
        int line = scanner.token().line();
        Type type;
        JMember memberDecl = null;
        if (have(VOID)) {
            mods.add(ABSTRACT.image());//in piazza, swami said to add abstract to method mods
            if(!mods.contains(PUBLIC.image())){
                mods.add(PUBLIC.image()); //implicitly add public to the list of modifiers for interface methods
            }
            ...
            if(have(THROWS)){
               ...
               mustBe(SEMI); //body should always be null right ?
                memberDecl = new JMethodDeclaration(line, mods, name, type,params, exceptions, null);
            }
            else {
                mustBe(SEMI);
                memberDecl = new JMethodDeclaration(line, mods, name, type, params, null, null);
            }
        }
        else {
            type = type();
 ...
 *                   | break SEMI
     *               | continue SEMI
     *               | DO statement WHILE parExpresion SEMI
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            JStatement st = statement();
            mustBe(WHILE);
            JExpression par = parExpression();
            mustBe(SEMI);
            return new JDoStatement(line, st, par);
        }
        else if (have(FOR)){ //for(init; condition; update) body || for(; ;)body
            mustBe(LPAREN);
        ...
        else if (have(IF)) { //if (condition) {} else{}
        ...
        else if(have(SWITCH)){ // switch (parexpression) {  all the cases and default - switchBlockStatementGroup}
            //from jSwitchstatement constructor
            ArrayList <SwitchStatementGroup> stmtGroup = new ArrayList<>();
            JExpression condition = parExpression();
            mustBe(LCURLY);
            //SwitchStatementGroup switchGroup = switchBlockStatementGroup();
            while(true){
                if( see(EOF) || see(RCURLY)){ //when using have and token is }, it will increment the token pointer,
                    break;                    //meaning that the following must be would fail
                }
                SwitchStatementGroup switchGroup2 = switchBlockStatementGroup();
                stmtGroup.add(switchGroup2);
            }
            mustBe(RCURLY);
            return new JSwitchStatement(line, condition, stmtGroup);
        }
        else if(have(THROW)){
            JExpression e = expression();
            mustBe(SEMI);
            return  new JThrowStatement(line, e);
        }
        else if (have(TRY)){ // TRY block { CATCH LPAREN formalParameter RPAREN block} [FINALLY block]
            ...
            //int line, JBlock tryBlock, ArrayList<JFormalParameter> parameters,
    ...
    /**
     * Parses a for loop initializer and returns an AST for it.
     *
     * <pre>
     ...
     * </pre>
     *
     * @return an AST for a for loop initializer .
     */
    /* statementExpression { COMMA statementExpression } | type variableDeclaration */
   private ArrayList<JStatement> forInit(){
       ArrayList <JStatement> list = new ArrayList<JStatement>();
       if(seeLocalVariableDeclaration()){
           // the following code is from localVariableDeclarationStatement
           int line = scanner.token().line();
           Type type = type();
           ArrayList<JVariableDeclarator> vDecls = variableDeclarators(type);
           //mustBe(SEMI);
           JVariableDeclaration obj1 = new JVariableDeclaration(line, vDecls);
           list.add(obj1);
       }
       else {
           readCommas();
    ...
    /**
     * Parses a for loop update and returns an AST for it.
     *
     * <pre>
     ...
     * </pre>
     *
     * @return an AST for a for loop update .
     */
    private ArrayList<JStatement> forUpdate(){
        return readCommas();
   }
   /*forUpdate and forInit helper function
     returns a list of sattementExpressions
    */
    private ArrayList<JStatement> readCommas(){
       boolean more  = true;
       ArrayList <JStatement> list = new ArrayList<JStatement>();
        JStatement st = statementExpression();
        list.add(st);
       while(more){
           if(have(COMMA)){//for(int i=0,w=0;i<size;i++)
               JStatement s = statementExpression();
               list.add(s);
           }
           else{
               more = false;
     ...
     */
    private SwitchStatementGroup switchBlockStatementGroup(){ //case : expression
        ArrayList<JExpression> switchLabels  = new ArrayList<>(); //from SwitchStatementGroup constructor
        ArrayList<JStatement> block = new ArrayList<>();
        //System.err.println("Beginning OF LOOP- token is "+ scanner.token().image());
        //switchLabels.add(switchLabel());
        do{
            if(see(CASE) || see(DEFAULT)){
                //System.err.println( "do while loop : "+scanner.token().image());
                switchLabels.add(switchLabel());
            }
            else {
                break;
        ...
        while(!see(CASE) && !see(DEFAULT) && !see(RCURLY)){
            //System.err.println("switchblock while: " + scanner.token().image());
    ...
    /**
     * Parses a switchLabel and returns an AST for it.
     *
     * <pre>
 ...
 *                      | DEFLT COLON
     ...
     */
    private JExpression switchLabel() { //case exp :
        //System.out.println("token: " + " " +scanner.token().image());
        //System.err.println("switchLabel token: " + scanner.token().image());
        if (have(CASE)) {
            //System.err.println("switchLabel case: " + scanner.token().image());
            JExpression expr = expression();
            mustBe(COLON);
            return expr;
...
//            reportParserError(scanner.token().image()+" have double: "+have(DOUBLE));
//            reportParserError(scanner.token().image()+" have long: "+have(LONG));
...
*                                [ ( ALSHIFT_ASSIGN | AND_ASSIGN | ARSHIFT_ASSIGN | ASSIGN | DIV-ASSIGN
     ...
     */
    private JExpression conditionalExpression(){//james' piazza post
        int line = scanner.token().line();
        JExpression cor  = conditionalOrExpression();
        if(have(QUESTION)){
            JExpression exp = expression();
            mustBe(COLON);
            JExpression ce = conditionalExpression();
            return new JConditionalExpression(line, cor, exp, ce);
        }
        else{
            return cor;
        }
    ...
    /**
     * Parses a conditionalOrexpression and returns an AST for it.
     *
     * <pre>
    ...
    /**
     * Parses an or expression and returns an AST for it.
     *
     * <pre>
    ...
    /**
     * Parses a XOR expression and returns an AST for it.
     *
     * <pre>
    ...
    private boolean have(TokenKind sought) {
        if(scanner.token().image() == EOF.image()){
            reportParserError("EOF "+ sought.image());
        }
        if (see(sought)) {
            scanner.next();
            return true;
        } else {
            //reportParserError("false");
            return false;
        }
    }
    ...
    private void mustBe(TokenKind sought) {
        if (scanner.token().kind() == sought) {
            ...
            System.err.println(scanner.previousToken().image());
            System.err.println(scanner.token().image());
            isRecovered = false;
    ...
    private boolean seeBasicType() {
        return (see(BOOLEAN) || see(CHAR) ||see(DOUBLE) || see(INT) || see(LONG));//reserved words in tokenKind
    }
    ...
    seeReferenceType() {
        if (see(IDENTIFIER)) {
            ...
            scanner.recordPosition();
            if (have(BOOLEAN) || have(CHAR) || have(INT) || have(DOUBLE) || have(LONG)) { //stats wasnt working bc i didnt add oduble and long here
