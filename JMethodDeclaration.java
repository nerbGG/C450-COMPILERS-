...
class JMethodDeclaration extends JAST implements JMember {
    ...
    //exception string names
    ArrayList <String> stringExceptions ;
    ...
    public JAST analyze(Context context) {
        ...
        if(exceptions != null){
            stringExceptions = new ArrayList<>();
            for(TypeName e : exceptions){
                stringExceptions.add(e.jvmName());
            }
        }
        // Declare the parameters. We consider a formal parameter to be always initialized, via a
        // method call.
        //LocalVariableDefn defn = null;
        for (JFormalParameter param : params) { //skip offset for longs and doubles
           // System.err.println(param.name() + " "+ param.type());
            ...
            if(param.type() == Type.LONG || param.type() == Type.DOUBLE){
                this.context.nextOffset(); //this increments the offset, why do we need to skip an offset?, *ASK ON PIAZZA*
                // defn = new LocalVariableDefn(param.type(), this.context.nextOffset() + 1);
            }
            defn.initialize();
