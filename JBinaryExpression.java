...
class JMultiplyOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        ...
        else if(lhs.type() == rhs.type() && lhs.type() == Type.LONG){
            output.addNoArgInstruction(LMUL);
        }
        else if(lhs.type() == rhs.type() && lhs.type() ==Type.DOUBLE){
...
class JDivideOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        if(lhs.type() == rhs.type() && lhs.type() == Type.INT){ //maybe just use type instead of lhs.type
...
class JOrOp extends JBinaryExpression {
     ...
     */
    public void codegen(CLEmitter output) {
        lhs.codegen(output);//first int
        rhs.codegen(output);//second int
